...
COMMENT ON TABLE MortgagePlatform_Staging.STG_Credit_Bureau_Feed IS
    'Staging: External Credit Bureau API extract. One record per customer per bureau refresh date. Source system: Equifax/Illion bureau feed. Enriches domain entities: Customer, Loan. Key source for fraud signals (DEROG_MARKS, SERIOUS_CREDIT_IMPAIRMENT, CRED_SCORE_CHG) and churn risk indicators. NOTE: CRED_SCORE_CURR uses Equifax 0-1200 scale — different from FICO 300-850 scale in STG_Freddie_Origination.CREDIT_SCORE.';
